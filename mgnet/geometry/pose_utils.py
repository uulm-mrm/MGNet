...
# https://github.com/TRI-ML/packnet-sfm/blob/master/packnet_sfm/geometry/pose_utils.py

import torch

__all__ = ["euler2mat", "pose_vec2mat", "invert_pose"]
...
def euler2mat(angle):
    ...
    zeros = z.detach() * 0
    ones = zeros.detach() + 1
    zmat = torch.stack([cosz, -sinz, zeros, sinz, cosz, zeros, zeros, zeros, ones], dim=1).view(
        B, 3, 3
    )
    ...
    ymat = torch.stack([cosy, zeros, siny, zeros, ones, zeros, -siny, zeros, cosy], dim=1).view(
        B, 3, 3
    )
    ...
    xmat = torch.stack([ones, zeros, zeros, zeros, cosx, -sinx, zeros, sinx, cosx], dim=1).view(
        B, 3, 3
    )
